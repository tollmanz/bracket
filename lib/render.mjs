...
const PERSON_COLORS = {
  ...
  nacin: '#f8d800', brothernacin: '#b8b8f8', fathernacin: '#f878b8', thoronas: '#f8b800',
...
export const page = ({ heading, subtitle, active, bracket, body, marquee }) => `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
export const renderLeaderboard = (bracket) => {
  ...
  const rows = board.map((b, i) => {
    ...
    return `<tr class="${rank <= 3 ? 'rank-' + rank : ''}">
      ...
      <td class="player"><span class="sprite">${skaterSvg(color(b.person))}</span>
        <span><span class="pname">${esc(upper(b.person))}</span>${champYears}</span></td>
      <td class="num cups" title="${b.championships} championship(s)">${cups}</td>
      <td class="num">${b.roundsWon}</td>
      <td class="num pts">${b.totalPoints}</td>
      <td class="num">${b.seasons}</td>
    </tr>`;
  }).join('');
  ...
  return `<section class="panel">
    ...
    <table class="leaderboard">
      <thead><tr><th>RANK</th><th class="player">PLAYER</th><th class="num">CUPS</th><th class="num">ROUNDS WON</th><th class="num">TOTAL PTS</th><th class="num">SEASONS</th></tr></thead>
      <tbody>${rows}</tbody>
    </table>
...
const standingsSection = (board, rounds) => {
  ...
  const items = board.map((s, i) => {
    const rank = i + 1;
    const bd = roundNums.map((r) => `R${r}: ${s.byRound[r] || 0}`);
    return `<li class="${rank <= 3 ? 'rank-' + rank : ''}">
      ...
      <span class="r1">${esc(bd[0] || '')}</span>
      <span class="r2">${esc(bd.slice(1).join(' · '))}</span>
