...
// Non-color marker + screen-reader label per outcome (so meaning is not color-only).
const MARK = {
...
const roundSection = (round, roster, upper, rules) => {
  const head = `<tr><th scope="col">#</th><th scope="col" class="col-series">SERIES</th><th scope="col">RESULT</th>${
  ...
  const rows = round.series.map((series, i) => {
    ...
    const cells = roster.map((p) => {
      ...
      const [mk, lbl] = MARK[cls];
      const mark = mk ? `<span class="mark" aria-hidden="true">${mk}</span>` : '';
      const sr = lbl ? `<span class="sr-only"> ${lbl}</span>` : '';
      return `<td class="${cls}">${mark}${txt}${sr}</td>`;
    }).join('');
  ...
  let scoreTable = '';
  if (hasFinal) {
    const shead = `<tr><th scope="col" class="col-series">SERIES</th>${roster.map((p) => `<th scope="col" class="num">${esc(upper(p))}</th>`).join('')}</tr>`;
    ...
    scoreTable = `<h3>POINTS EARNED</h3><table class="score-table"><caption class="sr-only">${esc(round.name)} points earned by player</caption><thead>${shead}</thead><tbody>${srows}</tbody><tfoot>${foot}</tfoot></table>`;
  }

  return `<section class="panel">
    <h2><span aria-hidden="true">${ARROWS[round.round] || '►'}</span> ${esc(round.name.toUpperCase())}</h2>
    <table><caption class="sr-only">${esc(round.name)} picks and results</caption><thead>${head}</thead><tbody>${rows}</tbody></table>
    ${scoreTable}
  </section>`;
};
...
const legendSection = (rules) => {
  ...
  return `<section class="panel"><h2>LEGEND</h2>
    <div class="legend">
      <span class="hit-bonus"><span class="mark" aria-hidden="true">★</span> CORRECT + GAMES = FULL PTS</span>
      <span class="hit"><span class="mark" aria-hidden="true">✓</span> CORRECT TEAM = BASE PTS</span>
      <span class="miss"><span class="mark" aria-hidden="true">✗</span> WRONG = 0 PTS</span>
