...
const roundSection = (round, roster, upper, rules) => {
  ...
  if (hasFinal) {
    ...
    scoreTable = `<h3>POINTS EARNED</h3><div class="table-wrap"><table class="score-table"><caption class="sr-only">${esc(round.name)} points earned by player</caption><thead>${shead}</thead><tbody>${srows}</tbody><tfoot>${foot}</tfoot></table></div>`;
  }

  return `<section class="panel">
    <h2><span aria-hidden="true">${ARROWS[round.round] || '►'}</span> ${esc(round.name.toUpperCase())}</h2>
    <div class="table-wrap"><table><caption class="sr-only">${esc(round.name)} picks and results</caption><thead>${head}</thead><tbody>${rows}</tbody></table></div>
    ${scoreTable}
  </section>`;
};
